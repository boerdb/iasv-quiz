#!/usr/bin/env python3
"""Deploy iASV Quiz naar Next-server (192.168.1.32) via SSH/SFTP — zelfde patroon als museum-pwa."""
import os
import stat
import sys
import tarfile
...
REMOTE_DIR = os.environ.get("DEPLOY_DIR", "/var/www/iasv-quiz")
...
def main() -> None:
    env_content = f"""DATABASE_URL=mysql://admin:kerkpoort@{DB_HOST}:3306/iasv_quiz
...
"""
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        remote_tar = "/tmp/iasv-quiz-deploy.tar.gz"
    ...
    _, out, _ = run(ssh, "curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:3012/", check=False)
    print(f"\nHTTP check 3012: {out.strip()}")
    run(ssh, "pm2 list", check=False)
    print("\n=== Deploy klaar ===")
    print(f"App: http://192.168.1.32:3012")
    print(f"Cloudflare Tunnel: service http://127.0.0.1:3012")
    ssh.close()
